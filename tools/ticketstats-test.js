...
const metResolved = () => ({
  ...
  configItem: "App B",
  createdOn: "2026-08-10 09:00:00",
  assignTimeUtcIso: "2026-08-10T09:00:00.000Z",
  acknTimeUtcIso: "2026-08-10T09:05:00.000Z",
...
test("counts tickets per state, most common first", () => {
  const stats = buildTicketStats(
    [
      openIncident("In Progress"),
      openIncident("In Progress"),
      openIncident("On Hold"),
      breachedResolved()
    ],
    fmt
  );
  ...
  assert.deepEqual(
    stats.types[0].states.map((s) => `${s.state}:${s.count}`),
...
test("groups states under ticket-type headings, types by descending count", () => {
  ...
  assert.deepEqual(
    stats.types.map((t) => `${t.type}:${t.count}`),
    ["Incident:2", "Problem Record:1"]
  );
  assert.deepEqual(
    stats.types[1].states.map((s) => `${s.state}:${s.count}`),
