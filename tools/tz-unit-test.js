...
check(
  "positive offset +5:30 (IST)",
  (() => {
    const off = detectSnOffsetMs([
      { openedAt: "2026-08-23 11:36:35", openedAtRaw: "2026-08-23 06:06:35" }
    ]);
...
check(
  "negative offset -7:00 (PDT)",
  (() => {
    const off = detectSnOffsetMs([
      { openedAt: "2026-08-23 06:06:35", openedAtRaw: "2026-08-23 13:06:35" }
    ]);
    return off === -7 * 3600e3;
  })(),
...
check(
  "zero offset (UTC profile)",
  (() => {
    const off = detectSnOffsetMs([
      { openedAt: "2026-08-23 06:06:35", openedAtRaw: "2026-08-23 06:06:35" }
    ]);
    return off === 0;
  })(),
  ""
...
check(
  "garbage pair skipped, second row used",
  (() => {
    const off = detectSnOffsetMs([
...
check(
  "median over many pairs (one outlier ignored)",
  (() => {
    const rows = [
...
const winter = {
  openedAt: "2021-01-15 05:04:14",
  openedAtRaw: "2026-01-15 13:04:14".replace("2026", "2021")
};
check(
  "summer row -> -7h even with winter global fallback",
  rowOffsetMs(summer, -8 * 3600e3) === -7 * 3600e3
);
check(
  "winter row -> -8h even with summer global fallback",
  rowOffsetMs(winter, -7 * 3600e3) === -8 * 3600e3,
  rowOffsetMs(winter, -7 * 3600e3)
);
check(
  "row without pair falls back to global",
  rowOffsetMs({ openedAt: "" }, -7 * 3600e3) === -7 * 3600e3
);

console.log("== non-ISO display formats (org instances) ==");
check(
  "dd-MM-yyyy display parses (iagprod style, +5:30 Kolkata)",
  pairOffsetMs("10-08-2026 11:06:40", "2026-08-10 05:36:40") === 19800e3,
  pairOffsetMs("10-08-2026 11:06:40", "2026-08-10 05:36:40")
);
check(
  "MM/dd/yyyy display parses (US profile)",
  pairOffsetMs("08/10/2026 11:06:40 PM", "2026-08-10 17:36:40") === 19800e3,
  pairOffsetMs("08/10/2026 11:06:40 PM", "2026-08-10 17:36:40")
);
check(
  "dd.MM.yyyy display parses",
...
check(
  "shift -7h stays same day",
  fmtWithOffset(T0, -7 * 3600e3) === "2026-08-22 06:42:14",
  fmtWithOffset(T0, -7 * 3600e3)
);
check(
  "shift +5:30 rolls to next day",
  fmtWithOffset(T0, 5.5 * 3600e3) === "2026-08-22 19:12:14",
  fmtWithOffset(T0, 5.5 * 3600e3)
);
check(
  "shift +9h rolls month edge",
  fmtWithOffset("2026-08-31T20:00:00Z", 9 * 3600e3) === "2026-09-01 05:00:00"
);
