...
if (MSR_DEFAULT_LISTS.queue.length !== 36) {
  failed++;
  console.log(`  FAIL queue count ${MSR_DEFAULT_LISTS.queue.length} want 36`);
} else console.log("  ok  queue count 36");
if (MSR_DEFAULT_LISTS.subCategory.length !== 33) {
  failed++;
  console.log(`  FAIL subCategory count ${MSR_DEFAULT_LISTS.subCategory.length} want 33`);
} else console.log("  ok  subCategory count 33");
if (MSR_DEFAULT_LISTS.rootCause.Incident.length !== 24) {
  failed++;
  console.log(`  FAIL incident rc count ${MSR_DEFAULT_LISTS.rootCause.Incident.length} want 24`);
} else console.log("  ok  incident root-cause count 24");
if (MSR_DEFAULT_LISTS.rootCause.P_Ticket.length !== 12) {
  failed++;
  console.log(`  FAIL p_ticket rc count`);
} else console.log("  ok  p_ticket root-cause count 12");
check("RFS list empty in source workbook", MSR_DEFAULT_LISTS.rootCause.RFS, []);

console.log("\n== mergeMsrLists ==");
check(
  "null override returns defaults",
  mergeMsrLists(null).resolution,
  MSR_DEFAULT_LISTS.resolution
);
check("partial array replaces whole list", mergeMsrLists({ status: ["Closed"] }).status, [
  "Closed"
]);
check(
  "untouched lists survive partial override",
  mergeMsrLists({ status: ["Closed"] }).opCo,
  MSR_DEFAULT_LISTS.opCo
);
check("rootCause deep merge", mergeMsrLists({ rootCause: { RFS: ["Custom"] } }).rootCause, {
  Incident: MSR_DEFAULT_LISTS.rootCause.Incident,
  RFS: ["Custom"],
  P_Ticket: MSR_DEFAULT_LISTS.rootCause.P_Ticket
});
check(
  "dupes and blanks dropped case-insensitively",
  mergeMsrLists({ opCo: ["BA", " ba ", "", "IB"] }).opCo,
  ["BA", "IB"]
);
...
check(
  "temporary wording",
  normResolution("Temporary fix until vendor patch"),
  "Workaround solution"
);
...
check(
  ...
  normResolution("Vendor hotfix applied"),
  "Vendor hotfix applied"
);
...
check(
  "root cause via msrType(PRB) is the P_Ticket list",
  rootCauseFor(MSR_DEFAULT_LISTS.rootCause, msrType("PRB0001234")).includes("User error - data"),
  true
);
check(
  "root cause via msrType(SCTASK) is the RFS list",
...
check(
  "INC + Closed Complete",
  isClassifyEligible({ number: "INC1", state: "Closed Complete" }),
  true
);
check("REQ + Closed (RFS)", isClassifyEligible({ number: "REQ1", state: "Closed" }), true);
check(
  "SCTASK + Resolved (RFS)",
  isClassifyEligible({ number: "SCTASK1", state: "Resolved" }),
  true
);
check(
  "INC + In Progress -> false",
  isClassifyEligible({ number: "INC1", state: "In Progress" }),
  false
);
check("REQ + Open -> false", isClassifyEligible({ number: "REQ1", state: "Open" }), false);
check(
  "PRB + Closed -> false (not Incident/RFS)",
  isClassifyEligible({ number: "PRB1", state: "Closed" }),
  false
);
check(
  "PTASK + Resolved -> false",
  isClassifyEligible({ number: "PTASK1", state: "Resolved" }),
  false
);
...
check(
  ...
  rootCauseFor(MSR_DEFAULT_LISTS.rootCause, "Incident").includes("Application bug"),
  true
);
check(
  "root cause for P_Ticket includes User error - data",
  rootCauseFor(MSR_DEFAULT_LISTS.rootCause, "P_Ticket").includes("User error - data"),
  true
);
...
console.log("\n== parseDisplayMs ==");
check("ISO", parseDisplayMs("2026-08-10 05:36:40"), Date.UTC(2026, 7, 10, 5, 36, 40));
check("day-first dash", parseDisplayMs("10-08-2026 11:06:40"), Date.UTC(2026, 7, 10, 11, 6, 40));
check("day-first dot", parseDisplayMs("10.08.2026 11:06"), Date.UTC(2026, 7, 10, 11, 6, 0));
check("US slash", parseDisplayMs("08/10/2026 11:06:40 PM"), Date.UTC(2026, 7, 10, 23, 6, 40));
...
close("28:20:30 -> ~1.18090277778 days", hmsToDays("28:20:30"), (28 + 20 / 60 + 30 / 3600) / 24);
close("00:35:03 response SLA", hmsToDays("00:35:03"), (35 * 60 + 3) / 3600 / 24);
