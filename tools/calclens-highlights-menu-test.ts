...
function openMenu(): void {
  ...
  if (menu.classList.contains("hidden")) {
    document
      .getElementById("calclensMenuBtn")!
...
test("dropdown lists all nine rules with their labels", { timeout: 8000 }, async () => {
  openMenu();
  await flush();
  const labels = [...document.querySelectorAll("#calclensHlList label span")].map(
    (s) => s.textContent
  );
  ...
  const boxes = [
    ...document.querySelectorAll<HTMLInputElement>("#calclensHlList input[type=checkbox]")
  ];
  assert.ok(
...
test(
  "toggling a rule off persists it and updates the button indicator",
  { timeout: 8000 },
  async () => {
    openMenu();
    await flush();
    ...
    assert.ok(
      (btn.getAttribute("data-tip") ?? "").includes("1 highlight"),
      "tooltip mentions the hidden count"
    );
  }
);
...
test("Hide all unchecks every rule; Show all re-checks", { timeout: 8000 }, async () => {
  ...
  await flush();
  document
    .getElementById("calclensHideAll")!
    .dispatchEvent(new window.Event("click", { bubbles: true }));
  await flush();
  let boxes = [
    ...document.querySelectorAll<HTMLInputElement>("#calclensHlList input[type=checkbox]")
