...
import {
  extractTimelines,
  analyzeAll,
  extractEventsFromListHistory
} from "../core/timeline/phase2.ts";
...
check(
  "backdated group entry clamps to opened_at",
  extractTimelines(
    [
...
check(
  "normal entry after birth untouched",
  extractTimelines(
...
check(
  "no openedAt in ctx -> no clamp applied",
  extractTimelines(
...
check(
  "born-in-queue fallback still equals opened_at",
  extractTimelines([], { ...base, openedAtUtcRaw: "2026-03-13 08:00:00" }).assignTimeUtcIso,
  "2026-03-13T08:00:00.000Z"
);
check(
  "ackn eligibility unaffected by clamp (pre-birth assignment still counts)",
  (() => {
...
check(
  "assigned_to listed before same-epoch group entry still acks (equal times)",
  (() => {
    const t = extractTimelines(
      [
...
check(
  "group entry listed before same-epoch assigned_to also acks (equal times)",
  (() => {
    const t = extractTimelines(
      [
        ev("assignment_group", "Other Queue", "QA Queue Alpha", "2026-08-23 06:06:43"),
...
check(
  "group re-entry takes latest entry",
  extractTimelines(
    [
    ...
    { ...base, openedAtUtcRaw: "2026-08-23 06:06:45" }
  ).assignTimeUtcIso,
  "2026-08-23T06:07:03.000Z"
);
check(
  "first On Hold wins, double hold counted",
  (() => {
    const t = extractTimelines(
      [
...
check(
  "hold->resolve gives resumeSource Resolved",
  (() => {
    const t = extractTimelines(
      [
...
check(
  "latest In Progress wins (not first)",
  (() => {
    const t = extractTimelines(
      [
...
check(
  "resume from any state (not only from On Hold)",
  (() => {
    const t = extractTimelines(
      [
...
check(
  "suspend only while in queue (hold during OTHER ignored)",
  extractTimelines(
    [
    ...
    { ...base, openedAtUtcRaw: "2026-08-23 06:07:20" }
  ).suspendTimeUtcIso,
  null
);
check(
  "never held -> resume stays null",
  extractTimelines(
    [
      ev("assignment_group", "Other Queue", "QA Queue Alpha", "2026-08-23 06:06:20"),
      ev("state", "1", "6", "2026-08-23 06:06:25")
    ],
    { ...base, openedAtUtcRaw: "2026-08-23 06:06:20" }
  ).resumeTimeUtcIso,
  null
);
...
check(
  "full lifecycle with label values: ackn+hold+resume",
  (() => {
    const t = extractTimelines(
      [
  ...
  [
    "2026-08-23T06:07:38.000Z",
    "2026-08-23T06:07:44.000Z",
    "2026-08-23T06:07:46.000Z",
    "2026-08-23T06:07:53.000Z",
    "Resolved"
  ]
);
check(
  "label hold->resolved fallback still works",
  (() => {
    const t = extractTimelines(
      [
        ev("assignment_group", "Other Queue", "QA Queue Alpha", "2026-08-23 06:09:00"),
...
(() => {
  ...
  const auditRows = [
    {
      ...
      oldValue: "Other",
      newValue: "QA Queue Alpha",
      at: "2026-08-23 06:01:00"
    },
  ...
  const makeRec = (sysId, number, state) => ({
    sys_id: sysId,
    number,
    state,
    assignment_group: "QA Queue Alpha",
  ...
  const res2 = analyzeAll(problemRecs, { s4: auditRows }, stateMap, {
    ...queueCtx,
    tableName: "problem"
  });
...
(() => {
  ...
  const recA = {
    ...
    problem_state: { value: "103", display_value: "103" },
    opened_at: "2026-08-23 06:00:00",
    assignment_group: "X"
  };
...
check(
  "S-A: ack from prior stay recovered after re-entry",
  (() => {
    const t = extractTimelines(
      [
...
check(
  "S-B: ack in latest stay uses latest entry as assignTime",
  (() => {
    const t = extractTimelines(
      [
        ev("assignment_group", "Other", "QA Queue Alpha", "2026-09-01 08:01:00"),
...
check(
  "S-C: three stays — ack recovered from oldest stay",
  (() => {
    const t = extractTimelines(
      [
...
check(
  "S-F: no ack ever — assignTime equals latest queue entry",
  (() => {
    const t = extractTimelines(
      [
        ev("assignment_group", "Other", "QA Queue Alpha", "2026-09-01 08:00:00"),
...
check(
  "S-D: hold in first stay preserved when assignTime walks back",
  (() => {
    const t = extractTimelines(
      [
        ev("assignment_group", "Other", "QA Queue Alpha", "2026-09-01 08:00:00"),
...
check(
  "S-E: held both stays — suspend from first, resume from last hold",
  (() => {
    const t = extractTimelines(
      [
        ev("assignment_group", "Other", "QA Queue Alpha", "2026-09-01 08:00:00"),
...
check(
  "S-E2: hold only in later stay, ack in first stay — hold picked up",
  (() => {
    const t = extractTimelines(
      [
        ev("assignment_group", "Other", "QA Queue Alpha", "2026-09-01 08:00:00"),
...
check(
  "S-G: hold before assignTime is dropped",
  (() => {
    const t = extractTimelines(
      [
        ev("assignment_group", "Other", "QA Queue Alpha", "2026-09-01 08:00:00"),
...
check(
  "Bug 2b: stale suspend from prior stay dropped when assignTime resets to latest entry",
  (() => {
    const t = extractTimelines(
      [
        ev("assignment_group", "Other", "QA Queue Alpha", "2026-09-01 08:00:00"),
...
check(
  "enforceOrderingContract: suspend <= assignTime is nulled",
  (() => {
    const t = extractTimelines(
      [
...
check(
  "enforceOrderingContract: hold->resolved captured as resume source Resolved",
  (() => {
    const t = extractTimelines(
      [
        ev("assignment_group", "Other", "QA Queue Alpha", "2026-09-01 08:00:00"),
...
const mrun = (rows, snap, openedAt) =>
  extractTimelines(rows, {
    ...multi,
    queueName: snap,
    snapshotGroupName: snap,
    openedAtUtcRaw: openedAt
  });

check(
  "S1 multi-queue, acked in earlier of our queues (assign goes back)",
  (() => {
    const t = mrun(
      [
...
check(
  "S2 acked in the latest of our queues",
  (() => {
    const t = mrun(
      [
        ev("assignment_group", "OTHER", "QA Queue Alpha", "2026-01-01 09:00:00"),
...
check(
  "S3 acked-stay wins over a later un-acked our-queue stay",
  (() => {
    const t = mrun(
      [
        ev("assignment_group", "OTHER", "QA Queue Alpha", "2026-01-01 09:00:00"),
...
check(
  "S4 through our queues but never acked -> latest entry, ack null",
  (() => {
    const t = mrun(
      [
        ev("assignment_group", "OTHER", "QA Queue Alpha", "2026-01-01 09:00:00"),
...
check(
  "S5 ticket never entered any of our queues -> both null",
  (() => {
    const t = mrun(
      [
...
check(
  "S6 born in one of our queues, acked",
  (() => {
    const t = mrun(
...
check(
  "S7 ack epoch equals our-queue entry epoch",
  (() => {
    const t = mrun(
      [
        ev("assignment_group", "OTHER", "QA Queue Alpha", "2026-01-01 09:00:00"),
...
check(
  "BA ticket resolves assign+ack from the earlier acked AIRPORTOPS stay",
  (() => {
    const rows = [
...
console.log(
  "== analyzeAll: priority source (u_priority for incident, priority for problem/sc_task) =="
);
(() => {
  ...
  const inc = {
    ...
    state: "Closed",
    opened_at: "2026-08-23 06:00:00",
    u_priority: val("2 - High"),
    priority: val("3 - Moderate")
  };
  ...
  const emptyObj = {
    ...
    state: "Closed",
    opened_at: "2026-08-23 06:00:00",
    u_priority: { display_value: "", value: "" },
    priority: val("2 - High")
  };
  const emptyObjRow = analyzeAll([emptyObj], {}, stateMap, { ...ctx, tableName: "sc_task" })
    .rows[0];
  ...
  const emptyStrRow = analyzeAll([emptyStr], {}, stateMap, { ...ctx, tableName: "sc_task" })
    .rows[0];
