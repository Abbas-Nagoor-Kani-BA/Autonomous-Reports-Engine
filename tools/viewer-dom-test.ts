...
after(async () => {
  /* keep store for post-run inspection if needed */
});
...
test("column-scoped search, match modes, and case toggle", { timeout: 8000 }, async () => {
  ...
  const reset = async () => {
    setSel(colSel, "");
    setSel(modeSel, "contains");
    document
      .getElementById("searchCaseInsensitive")
  ...
  assert.match(
    ...
    /0 \/ 2/,
    "state column does not contain 'First'"
  );
  ...
  assert.match(
    document.getElementById("count").textContent,
    /1 \/ 2/,
    "Equals matches the one Closed row"
  );
  ...
  assert.match(
    document.getElementById("count").textContent,
    /1 \/ 2/,
    "notEquals keeps the non-Closed row"
  );
  ...
  setSearch("closed");
  document
    .getElementById("searchCaseSensitive")
    .dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
  await flush();
  assert.match(
    document.getElementById("count").textContent,
    /0 \/ 2/,
    "case-sensitive Equals rejects lowercase"
  );
  ...
  assert.match(
    document.getElementById("count").textContent,
    /2 \/ 2/,
    "both rows are Incident by displayed Type"
  );
...
test(
  "Columns menu drag-to-sort reorders columns, persists, then Reset order restores",
  { timeout: 8000 },
  async () => {
    const store = await import("../viewer/store.ts");
    const headerLabels = () =>
      [...document.querySelectorAll("#tbl thead th")].map((t) =>
    ...
    assert.ok(
      after.indexOf("Number") > after.indexOf(before[2]),
      "Number moved after its drop target"
    );
    assert.ok(store.getColOrder().length > 0, "order held in the ui store");
    assert.ok(
      Array.isArray(peek("viewerColOrder")) && peek("viewerColOrder").length > 0,
    ...
    document
      .getElementById("resetColOrderBtn")
      .dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
    await flush();
...
test(
  "grid body is permanently read-only (no inline editor on double-click)",
  { timeout: 8000 },
  async () => {
    const tr = document.querySelector("#tbl tbody tr");
    const prioIdx = [...document.querySelectorAll("#tbl thead th")].findIndex((t) =>
      t.textContent.toLowerCase().includes("priority")
    );
    const td = tr.children[prioIdx];
    assert.ok(
      td.classList.contains("calclens-cell"),
      "every body cell carries the calclens-cell class"
    );
...
test("Calclens date input does not persist until Save is clicked", { timeout: 8000 }, async () => {
  ...
  assert.ok(
    body.querySelector(".calclens-edit-input.tlDate"),
    "drawer offers a date editor for a derivation column"
  );
  ...
  assert.notEqual(
    peek("lastData").rows.find((r) => r.sysId === "aaa").assignTimeUtcIso,
    iso,
  ...
  const assignIdx = [...document.querySelectorAll("#tbl thead th")].findIndex(
    (t) => t.textContent.trim() === "Assign time"
  );
  assert.ok(
    cell.children[assignIdx].textContent.length > 0,
    "grid re-rendered with the edited timeline time"
  );
  calState.setCalclensMode(false);
});

test(
  "Calclens timeline pick stages a time and only Save persists + toasts",
  { timeout: 8000 },
  async () => {
    const calState = await import("../viewer/calclens-state.ts");
    const row = grid.findRowBySysId("aaa");
    ...
    const pickRow = [...tlRows].find((r) =>
      r.querySelector(".calclens-tl-change").textContent.includes("In Progress")
    );
    ...
    await flush();
    assert.equal(
      document.querySelector(".calclens-tl-row.selected .calclens-tl-time").textContent,
      "05-08-2026 11:15:00",
      "clicking a timeline row highlights it as selected"
    );
    ...
    body
      .querySelector(".calclens-edit-save")
      .dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
    await new Promise((r) => setTimeout(r, 500)); // debounce scheduleSave
    const stored = peek("lastData").rows.find((r) => r.sysId === "aaa");
...
test(
  "clicking a key-moment chip jumps to the derived cell and opens its editor",
  { timeout: 8000 },
  async () => {
    const calState = await import("../viewer/calclens-state.ts");
    ...
    row.activity = [
      {
        ...
        o: "APPSUP_TEST",
        n: "APPSUP_TEST",
        atEpoch: Date.UTC(2026, 7, 25, 9, 15, 0)
      },
    ...
    assert.equal(
      selection.getSelFocus()?.key,
      "assignTimeUtcIso",
      "grid focus jumped to the Assign time cell"
    );
    assert.equal(selection.getSelFocus()?.sysId, "aaa", "same row is focused");
    const body2 = document.getElementById("calclensBody");
    assert.ok(
      body2.querySelector(".calclens-edit-input.tlDate"),
      "derived time editor is now shown"
    );
    calState.setCalclensMode(false);
  }
);
...
test(
  "Calclens groups same-timestamp changes under one timeline node",
  { timeout: 8000 },
  async () => {
    const calState = await import("../viewer/calclens-state.ts");
    const row = grid.findRowBySysId("aaa");
    ...
    assert.deepEqual(
      groups.map(String).sort(),
      ["1", "2"],
      "the shared timestamp appears once with two changes"
    );
    const clockIcons = body.querySelectorAll(
      ".calclens-tl-row .calclens-tl-icn[data-icon='clock']"
    );
...
test(
  "Calclens discards an unsaved pick when moving to the next cell",
  { timeout: 8000 },
  async () => {
    const calState = await import("../viewer/calclens-state.ts");
    ...
    row.activity = [
      {
        ...
        o: "APPSUP_TEST",
        n: "PAYMENTS",
        atEpoch: Date.UTC(2026, 7, 5, 9, 30, 0)
      }
    ];
    calState.setCalclensMode(true);
    ...
    await new Promise((r) => setTimeout(r, 500)); // debounce scheduleSave
    assert.notEqual(
      peek("lastData").rows.find((r) => r.sysId === "bbb").assignTimeUtcIso,
      new Date(2026, 7, 5, 9, 30, 0).toISOString(),
...
test(
  "Calclens drawer shows a choice picker but not a date editor for solutionType",
  { timeout: 8000 },
  async () => {
    const calState = await import("../viewer/calclens-state.ts");
    calState.setCalclensMode(true);
    ...
    assert.ok(
      section.querySelector("input.calclens-edit-input:not(.tlDate)"),
      "choice column renders a picker input"
    );
    assert.equal(
      section.querySelector(".calclens-edit-input.tlDate"),
      null,
      "no date editor for a choice column"
    );
    calState.setCalclensMode(false);
  }
);
...
test(
  "Calclens drawer does not offer editing for non-derivation columns",
  { timeout: 8000 },
  async () => {
    const calState = await import("../viewer/calclens-state.ts");
    calState.setCalclensMode(true);
    ...
    const body = document.getElementById("calclensBody");
    assert.equal(
      body.querySelector(".calclens-edit"),
      null,
      "no edit section for a read-only column"
    );
    calState.setCalclensMode(false);
  }
);
...
test("Calclens ON flags attention cells with a marker and tooltip", { timeout: 8000 }, async () => {
  ...
  assert.ok(
    marked.length >= 2,
    "hinted cells show the attention marker (rootCause + solutionType)"
  );
  for (const cell of marked) {
    const tip = cell.getAttribute("data-tip") ?? "";
    assert.ok(
      tip.includes("Needs attention") && tip.includes("Missing plan data"),
      "marked cell tooltip lists the fired rule and detail"
    );
  }
  calState.setCalclensMode(false);
  grid.render();
  await flush();
  assert.equal(
    document.querySelector("#tbl tbody td.attention-mark"),
    null,
    "markers cleared when Calclens is toggled off"
  );
});

test(
  "disabling a highlight suppresses the mark but keeps the tooltip",
  { timeout: 8000 },
  async () => {
    const calState = await import("../viewer/calclens-state.ts");
    ...
    assert.equal(
      ...
      null,
      "disabled highlight paints no mark"
    );
    ...
    await flush();
    assert.ok(
      document.querySelectorAll(`#tbl tbody tr[data-sys-id="aaa"] td.attention-mark`).length >= 2,
...
test("tooltip hides when the hovered cell is re-rendered", { timeout: 8000 }, async () => {
  ...
  cell.dispatchEvent(
    new window.MouseEvent("mouseover", { bubbles: true, clientX: 300, clientY: 100 })
  );
  const tipIn = () => {
  ...
  document.body.dispatchEvent(
    new window.MouseEvent("mouseover", { bubbles: true, clientX: 5, clientY: 5 })
  );
  await flush();
...
test(
  "classifyRows degrades to the scorer with a notice when the ML model is missing",
  { timeout: 8000 },
  async () => {
    ...
    assert.ok(
      run.notice && run.notice.includes("not downloaded"),
      "reports the missing model to the user"
    );
...
test(
  "switching classification mode re-runs on already-loaded data (mode is in the run guard)",
  { timeout: 8000 },
  async () => {
    ...
    assert.equal(
      row.rootCause,
      "Firewall",
      "heuristic run categorised the root cause from the label"
    );
    assert.equal(
      row.solutionType,
      "Permanent solution",
      "heuristic run categorised the solution type"
    );
    ...
    assert.notEqual(
      row.__classFp,
      fpBefore,
      "the run context (fp incl. mode) changed, so it re-ran"
    );
  }
);

test(
  "single-cell selection + arrow navigation focuses the selected cell",
  { timeout: 8000 },
  async () => {
    const calState = await import("../viewer/calclens-state.ts");
    ...
    assert.ok(
      !numTd.classList.contains("numLink"),
      "number cell is a plain read-only cell (no detail popup)"
    );
    assert.equal(document.querySelector(".ticketPop"), null, "no ticket popup exists anymore");
    const numIdx = [...document.querySelectorAll("#tbl thead th")].findIndex(
      (t) => t.textContent.trim() === "Number"
    );
    ...
    await flush();
    const selCol = [
      ...document.querySelector(`#tbl tbody tr[data-sys-id="aaa"]`).children
...
test(
  "derived duration columns render HMS from the timeline stamps",
  { timeout: 8000 },
  async () => {
    const row = grid.currentRows()[0];
    Object.assign(row, {
    ...
    assert.equal(
      cells[idxOf("Time to ackn")].textContent,
      "0:30:00",
      "assign->ackn rendered as HMS"
    );
    assert.equal(
      cells[idxOf("Time to resolve")].textContent,
      "8:00:00",
      "assign->resolve rendered as HMS"
    );
    assert.equal(
      cells[idxOf("Suspend total")].textContent,
      "1:15:00",
      "suspend total rendered as HMS"
    );
  }
);

test(
  "clicking a body cell reports it to Calclens (single-cell focus)",
  { timeout: 8000 },
  async () => {
    const calState = await import("../viewer/calclens-state.ts");
    calState.setCalclensMode(true);
    ...
    assert.equal(
      document.querySelector(".ticketPop"),
      null,
      "cell click never opens a ticket popup"
    );
    calState.setCalclensMode(false);
  }
);
...
test("summary SLA tab renders counts and persists summarySla", { timeout: 8000 }, async () => {
  ...
  assert.deepEqual(
    stored.summarySla.incidentTotals,
    liveTotals,
    "incident totals by severity (closed/resolved incidents only)"
  );

  tabSummary.dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
  await flush();
  assert.ok(
    !document.getElementById("summaryWrap").classList.contains("hidden"),
  ...
  await flush();
  assert.ok(
    document.getElementById("wrap").classList.contains("hidden") === false,
    "tickets view restored"
  );
  assert.ok(
    document.getElementById("summaryWrap").classList.contains("hidden"),
    "summary panel hidden again"
  );
});
...
test("ticket stats icon opens a popup with state and SLA sections", { timeout: 8000 }, async () => {
  ...
  document
    .getElementById("ticketStatsClose")
    .dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
  await flush();
...
test("summary SLA tab reflects the active search filter", { timeout: 8000 }, async () => {
  ...
  assert.equal(
    totalFor("Within 10 working days"),
    "0",
    "P4 resolve total excludes the open (In Progress) ticket"
  );
  assert.equal(totalFor("Within 1 hour"), "0", "P1 resolve total empty");
  assert.equal(
    totalFor("Within 3 business hours"),
    "0",
    "P4 respond total excludes the open ticket"
  );
  assert.match(document.getElementById("sumMeta").textContent, /P1 0, P2 0, P3 0, P4 0/);
  assert.match(document.getElementById("sumMeta").textContent, /showing 1 of 2 tickets/);
  tabTickets.dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
  search.value = "";
  search.dispatchEvent(new window.Event("input", { bubbles: true }));
  await flush();
...
test("selecting 'Separate files' with no groups opens the CI dialog and reverts", async () => {
  ...
  assert.ok(
    !document.getElementById("ciModal").classList.contains("hidden"),
    "CI dialog opened to configure groups"
  );
  document
    .getElementById("ciClose")
    .dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
  await flush();
...
test(
  "split export writes one file per CI group (serialized downloads)",
  { timeout: 8000 },
  async () => {
...
<row r="2"><c r="A2" s="4"/><c r="E2" s="7" t="inlineStr"><is><t>INCOLD</t></is></c></row>
    ...
    assert.equal(
      dl.every((d) => d.saveAs === false),
      true,
      "no save-as prompts for split files"
    );
  }
);
...
test(
  "cell copy/paste/undo are disabled: no fill handle and body edits are blocked",
  { timeout: 8000 },
  async () => {
    const selModule = await import("../viewer/selection.ts");
    assert.equal(document.getElementById("fillHandle"), null, "fill handle element removed");
    assert.equal(
      document.querySelector("#tbl.selecting"),
      null,
      "no range-select mode class exists"
    );
    await selModule.setSelPoint("aaa", "shortDescription", false);
    await flush();
    document.body.dispatchEvent(
      new window.KeyboardEvent("keydown", { key: "c", ctrlKey: true, bubbles: true })
    );
    await flush();
    document.body.dispatchEvent(
    ...
    await flush();
    assert.equal(
      grid.findRowBySysId("aaa").shortDescription,
      "First ticket",
      "Ctrl+C/V/Z leave the body untouched"
    );
  }
);
...
test(
  "dragging a column resize handle updates and persists the width",
  { timeout: 8000 },
  async () => {
    const store = await import("../viewer/store.ts");
    store.setColWidths({});
...
test("split preview icon session-filters the grid by CI group", { timeout: 8000 }, async () => {
  ...
  document
    .getElementById("splitPreviewBtn")
    .dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
  await flush();
  ...
  const payRow = [...document.querySelectorAll("#splitPreviewBody .splitRow")].find(
    (r) => r.querySelector(".splitName").textContent === "Payments"
  );
  assert.ok(payRow, "Payments row present");
  payRow
    .querySelector(".splitView")
    .dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
  await flush();
...
test(
  "edit mode opens the column editor on cell select, listing that column across the view",
  { timeout: 8000 },
  async () => {
...
test(
  "buildTimeline merges notes and field changes newest-first and marks key moments",
  { timeout: 8000 },
  async () => {
    ...
    const row = {
      ...
      assignmentGroup: "APPSUP_TEST",
      workNotes:
        "2026-08-01 09:20:00 - Jane Doe\nLooking into it\n\n2026-08-01 10:00:00 - Jane Doe\nRoot cause found",
      closeNotes: "",
      activity: [
        {
          atEpoch: Date.parse("2026-08-01T09:00:00Z"),
          f: "assignment_group",
          o: "",
          n: "APPSUP_TEST"
        },
    ...
    for (let i = 1; i < epochs.length; i++)
      assert.ok(epochs[i] <= epochs[i - 1], "newest-first order");
...
test(
  "buildTimeline interleaves notes and field changes across an instance offset",
  { timeout: 8000 },
  async () => {
    const activity = await import("../viewer/activity.ts");
...
test(
  "buildTimeline splits a multi-entry note blob (dd-MM-yyyy) and interleaves each with field changes",
  { timeout: 8000 },
  async () => {
    const activity = await import("../viewer/activity.ts");
    const row = {
      ...
      activity: [
        {
          atEpoch: Date.parse("2026-09-03T13:43:50Z"),
          f: "state",
          o: "In Progress",
          n: "Resolved"
        },
    ...
    assert.ok(
      tl.every((t) => Number.isFinite(t.epoch)),
      "every note got a parsed timestamp"
    );
    ...
    const fcCard = groups.find((g) =>
      g.items.some((i) => i.kind === "fc" && i.label === "Assignment group")
    );
    assert.ok(
      fcCard.items.every((i) => i.kind === "fc"),
      "field-change card has no notes"
    );
...
test("groupTimeline never merges notes/comments even at the same timestamp", async () => {
  ...
  assert.ok(
    noteCards.every((g) => g.items.length === 1),
    "each note/comment card holds exactly one item"
  );
