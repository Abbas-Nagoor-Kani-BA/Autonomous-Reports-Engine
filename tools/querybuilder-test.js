...
check(
  "neq strips quotes",
  encodeConditions([{ field: "number", oper: "neq", value: "INC'001" }]),
  "number!=INC001"
);
check(
  "contains",
  encodeConditions([{ field: "short_description", oper: "contains", value: "printer" }]),
  "short_descriptionLIKEprinter"
);
check(
  "startsWith",
  encodeConditions([{ field: "number", oper: "startsWith", value: "INC" }]),
  "numberSTARTSWITHINC"
);
...
check(
  "planned start between (start_date)",
  encodeConditions([
    { field: "start_date", oper: "between", value: "2026-09-03", value2: "2026-09-03" }
  ]),
  "start_dateBETWEENjavascript:gs.dateGenerate('2026-09-03','00:00:00')@javascript:gs.dateGenerate('2026-09-03','23:59:59')"
);
check(
  "planned end after (end_date)",
...
check(
  "skip empty first no leading ^OR",
  encodeConditions([
...
checkTrue(
  "conditions before queue scope",
  q.startsWith("assigned_toISEMPTY^state=2^assignment_group.nameIN"),
  q
);
check(
  "queue IN list encoded",
  q,
  "assigned_toISEMPTY^state=2^assignment_group.nameINQueue A,Queue B"
);
...
check(
  "a real condition is constrained",
  hasQueryConstraint({ conditions: [{ field: "number", oper: "startsWith", value: "INC" }] }),
  true
);
...
check(
  "an incomplete condition row is unconstrained",
  hasQueryConstraint({ conditions: [{ field: "number", oper: "", value: "" }] }),
  false
);
check(
  "a top-level OR rawQuery still counts as constrained (never throws)",
  hasQueryConstraint({ rawQuery: "priority=1^ORpriority=2" }),
  true
);
