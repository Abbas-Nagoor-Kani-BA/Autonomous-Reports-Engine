#!/usr/bin/env node
import { detectSnOffsetMs, rowOffsetMs, fmtWithOffset } from "../core/timeline/sntime.ts";
...
(async () => {
  ...
  const recs = await api("/api/now/table/incident", {
    ...
    sysparm_fields: "sys_id,number,opened_at",
    sysparm_display_value: "all",
    sysparm_limit: 100
  });
  const global = detectSnOffsetMs(
    recs.map((r) => ({
      openedAt: r.opened_at?.display_value || "",
      openedAtRaw: r.opened_at?.value || ""
    }))
  );
  ...
  console.log(
    `  ${checked} tickets: rendered opened_at === SN display value ${fail ? "" : "(exact match)"}`
  );
  ...
  for (const r of oob) {
    ...
    const deltaMin = Math.round(
      (Date.parse(oldWay.replace(" ", "T") + "Z") - Date.parse(disp.replace(" ", "T") + "Z")) / 60e3
    );
    console.log(
      `  ${val(r.number)} SN-display=${disp}  per-row=${okNow ? "MATCH" : `FAIL (${rendered})`}  ${deltaMin ? `(old fixed-offset way was off by ${Math.abs(deltaMin)}min)` : "(fixed-offset also matched)"}`
    );
    if (!okNow) fail++;
  }
