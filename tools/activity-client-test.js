...
const mkClient = (t, diags) =>
  new ServiceNowClient("https://x.service-now.com", {
    transport: t,
...
(async () => {
  ...
  check(
    "only list_history.do called",
    t.calls.every((u) => u.includes("/list_history.do"))
  );
  check(
    "probe reused - one call per ticket",
    t.calls.filter((u) => u.includes("/list_history.do")).length === 2
  );
  check("no timestamp param sent (full dump)", !t.calls.some((u) => /sysparm_timestamp/.test(u)));
  check(
    "incident_state mapped to state",
    got.AAA.some((e) => e.field === "state" && e.newValue === "In Progress")
  );
  check(
    "all parsed changes kept when no filter fields requested",
    got.AAA.some((e) => e.field === "state") && got.AAA.some((e) => e.field === "close_notes")
  );
  const filtered = await c.fetchTimelineEvents(["AAA"], ["state"], () => {});
  check(
    "explicit field list still filters",
    filtered.AAA.every((e) => e.field === "state")
  );
  ...
  check(
    ...
    t.calls.some((u) => u.includes("/list_history.do")) &&
      t.calls.some((u) => u.includes("/api/now/v1/activity/stream"))
  );
  check("stream events parsed after fallback", got.AAA.length === 2);
  check(
    "fallback note emitted",
    diagsA.some(
      (d) => d.kind === "warn" && /using \/api\/now\/v1\/activity\/stream/.test(d.note || "")
    )
  );
  ...
  check(
    "empty-feed note emitted",
    diagsE.some((d) => d.kind === "warn" && /yielded no field changes/.test(d.note || ""))
  );
