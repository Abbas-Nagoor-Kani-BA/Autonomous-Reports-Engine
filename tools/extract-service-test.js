...
check(
  "stats over 3 rows: only solutionType fills (rootCause is the classifier's job)",
  svc.applyExtraction([
...
check(
  ...
  svc.applyExtraction([
    {
      closeNotes: "root cause: bad cert",
...
check(
  "fills only solutionType, never touches rootCause",
  (() => {
    const row = { closeNotes: "resolution type: Workaround", rootCause: "full disk" };
    const out = svc.applyExtraction([row]);
    return (
      String(row.solutionType) === "Workaround solution" &&
      String(row.rootCause) === "full disk" &&
      out.filled === 1 &&
      row.parseReview === undefined
    );
  })(),
  true
);
...
check(
  "RCA analysis narrative is NOT written into rootCause (left for the classifier)",
  (() => {
    const row = {
      closeNotes:
        "Analysis (Root Cause): the cache directory filled up completely and corrupted the files"
    };
    const out = svc.applyExtraction([row]);
...
check(
  "solutionType still flags parseReview on medium confidence",
  (() => {
...
check(
  "smoke: sane default when no notes exist",
  svc.applyExtraction([{}, { closeNotes: null }, { closeNotes: "   " }]),
  { total: 3, withNotes: 0, filled: 0 }
);

check(
  "numeric closeNotes never crash (hardened)",
  (() => {
    let out = null;
    try {
