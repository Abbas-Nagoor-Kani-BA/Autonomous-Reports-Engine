...
check("empty row -> all empty", computeDurations({}), {
  assignToAckn: "",
  assignToResolve: "",
  suspendTotal: ""
});
check(
  "assign only -> resolve empty",
  computeDurations({ assignTimeUtcIso: "2026-01-05T09:00:00.000Z" }),
  { assignToAckn: "", assignToResolve: "", suspendTotal: "" }
);
check(
  "ackn before assign -> empty (inverted)",
  computeDurations({
    assignTimeUtcIso: "2026-01-05T09:00:00.000Z",
...
check(
  "unparseable stamps -> empty, no throw",
  computeDurations({
