...
const changes = [
  ...
  {
    number: "CHG004",
    short_description: "open d",
    state: "Implement",
    cmdb_ci: "OPS (prd)",
    start_date: "2026-08-20 09:00:00",
    end_date: "2026-08-27 09:00:00"
  },
  // start_date in NEXT week + end_date not in last week -> in none of the three buckets
  {
    number: "CHG005",
    short_description: "old e",
    state: "Closed",
    cmdb_ci: "X (prd)",
    start_date: "2026-09-08 09:00:00",
    end_date: "2026-09-12 09:00:00"
  },
  // u_failure flag failed, end_date in last week
  {
    number: "CHG006",
    short_description: "fail f",
    state: "Closed",
    cmdb_ci: "Y (prd)",
    start_date: "2026-08-21 09:00:00",
    end_date: "2026-08-28 09:00:00",
    u_failure: "true"
  },
  // end_date in last week but Cancelled -> excluded from every bucket
  {
    number: "CHG007",
    short_description: "cancel g",
    state: "Cancelled",
    cmdb_ci: "Z (prd)",
    start_date: "2026-08-22 09:00:00",
    end_date: "2026-08-29 09:00:00"
  }
];
const b = bucketChanges(changes, w);
check("implemented count", b.implemented.length, 2);
check(
  "implemented includes CHG001 (Closed)",
  b.implemented.some((r) => r.crNumber === "CHG001"),
  true
);
check(
  "implemented includes CHG004 (Implement, not yet closed)",
  b.implemented.some((r) => r.crNumber === "CHG004"),
  true
);
check("failed count", b.failed.length, 2);
check(
  "failed includes CHG002",
  b.failed.some((r) => r.crNumber === "CHG002"),
  true
);
check(
  "failed includes CHG006 (u_failure)",
  b.failed.some((r) => r.crNumber === "CHG006"),
  true
);
check("planned count", b.planned.length, 1);
check("planned cr", b.planned[0].crNumber, "CHG003");
check(
  "next-week change excluded",
  b.implemented.concat(b.planned, b.failed).some((r) => r.crNumber === "CHG005"),
  false
);
check(
  "cancelled change excluded",
  b.implemented.concat(b.planned, b.failed).some((r) => r.crNumber === "CHG007"),
  false
);
...
const edgeChanges = [
  {
    number: "EDGE1",
    short_description: "monday 00:00",
    state: "Closed",
    cmdb_ci: "A (prd)",
    start_date: "2026-08-17 09:00:00",
    end_date: "2026-08-24 00:00:00",
    review_status: "Successful"
  },
  {
    number: "EDGE2",
    short_description: "sunday 23:59",
    state: "Closed",
    cmdb_ci: "B (prd)",
    start_date: "2026-08-17 09:00:00",
...
check(
  "monday-00:00 edge implemented",
  eb.implemented.some((r) => r.crNumber === "EDGE1"),
  true
);
check(
  "sunday-23:59 edge implemented",
  eb.implemented.some((r) => r.crNumber === "EDGE2"),
  true
);
...
const incidents = [
  {
    ...
    state: "Resolved",
    cmdb_ci: "RMS (prd)",
    short_description: "p1 last week",
    resolved_at: "2026-08-25 12:00:00",
    close_notes: "root cause X"
  },
  {
    number: "INC002",
    priority: "2 - High",
    state: "Closed",
    cmdb_ci: "SYS (prd)",
    short_description: "p2 last week",
    resolved_at: "2026-08-29 08:00:00"
  },
  {
    number: "INC003",
    priority: "3 - Moderate",
    state: "Resolved",
    short_description: "p3 excluded",
    resolved_at: "2026-08-25 08:00:00"
  },
  {
    number: "INC004",
    priority: "1 - Critical",
    state: "Resolved",
    short_description: "p1 current week excluded",
    resolved_at: "2026-09-02 08:00:00"
  }
];
...
check(
  "p3 excluded",
  ki.some((r) => r.incidentNumber === "INC003"),
  false
);
check(
  "current week p1 excluded",
  ki.some((r) => r.incidentNumber === "INC004"),
  false
);
