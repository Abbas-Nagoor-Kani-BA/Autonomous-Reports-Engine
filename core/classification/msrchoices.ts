...
export type MsrListOverrides = Partial<Omit<MsrListSet, "rootCause" | "hints">> & {
  rootCause?: Partial<RootCauseLists>;
  hints?: Record<string, string[]>;
};
...
const MSR_DEFAULT_LISTS: MsrListSet = {
  ...
  subCategory: [
    "Cause",
    "Access Issue",
    "Covid Reduced Operations",
    "Data Request",
    "DB Issue",
    "Dependant Application failue - CAML",
    "Dependant Application failue - CIRRUS",
    "Dependant Application failue - FICO",
    "Dependant Application failue - Loreto",
    "Expected Behaviour",
    "FLY code issue",
    "In Progress",
    "Insufficient Info/logs",
    "Network Issue",
    "No issue found / Auto resolved",
    "Not related to SIP",
    "SIP CID Release",
    "SIP Infra - Batch",
    "SIP Infra - Battery",
    "SIP Infra - Certificate",
    "SIP Infra - Components",
    "SIP Infra - DSMS",
    "SIP Infra - False alerts",
    "SIP Infra - Filesystem",
    "SIP Infra - Prob runner",
    "SIP Infra - Queue pileup",
    "SIP Infra - Topic DB",
    "SIP Services issue",
    "SIP Services Release",
    "Third Party - Amadeus",
    "Third Party - SITA",
    "Third Party - Others",
...
function normResolution(v: unknown): string {
  ...
  if (
    /^temp\b|^temporary/.test(n) ||
    /\bmonitoring\b/.test(n) ||
    /\beducat/.test(n) ||
    /no issue found/.test(n)
  ) {
...
const STATUS_MAP: Array<[RegExp, string]> = [
  [
    /^(closed|resolved|cancelled|canceled|closed complete|closed incomplete|closed skipped)/i,
    "Closed"
  ],
 ...
 */
function isClassifyEligible(
  row: { number?: unknown; state?: unknown } | null | undefined
): boolean {
  if (!row) return false;
...
function rootCauseFor(
  rootCauseLists: RootCauseLists | null | undefined,
  typeLabel: unknown
): string[] {
...
function hmsToHours(v: unknown): number {
  ...
  return (
    parseInt(parts[0], 10) + parseInt(parts[1], 10) / 60 + (parseInt(parts[2], 10) || 0) / 3600
  );
}
